    ...
    <nav className="bg-white/75 backdrop-blur-lg shadow-sm sticky top-0 z-50 border-b border-gray-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          {/* Desktop Navigation */}
          <div className="hidden md:flex md:items-center md:justify-center md:flex-1 md:space-x-4">
              {loggedInUser ? (
                <NavLink to={loggedInUser.accountType === 'lender' ? '/lender' : '/'} className="text-gray-600 hover:text-green-700 hover:bg-green-50 px-3 py-2 rounded-md text-sm font-medium transition-colors" style={({ isActive }) => isActive ? activeLinkStyle : undefined}>
                  ড্যাশবোর্ড
                </NavLink>
              ) : (
                <>
                  <NavLink to="/" className="text-gray-600 hover:text-green-700 hover:bg-green-50 px-3 py-2 rounded-md text-sm font-medium transition-colors" style={({ isActive }) => isActive ? activeLinkStyle : undefined}>
                    কৃষক
                  </NavLink>
              ...
              {(!loggedInUser || loggedInUser.accountType !== 'lender') && (
                <NavLink to="/rent" className="text-gray-600 hover:text-green-700 hover:bg-green-50 px-3 py-2 rounded-md text-sm font-medium transition-colors" style={({ isActive }) => isActive ? activeLinkStyle : undefined}>
                  ভাড়া করুন
                </NavLink>
              )}
          ...
          <div className="hidden md:flex items-center justify-end space-x-4">
              {loggedInUser && (
                <>
                  <NavLink to="/profile" className="text-gray-600 hover:text-green-700 hover:bg-green-50 px-3 py-2 rounded-md text-sm font-medium transition-colors" style={({ isActive }) => isActive ? activeLinkStyle : undefined}>
                    আমার প্রোফাইল
                  </NavLink>
                  <button onClick={logout} className="bg-red-500 text-white px-3 py-1 rounded-md text-sm font-medium hover:bg-red-600 transition-colors">
                    লগ আউট
                  </button>
                </>
              )}
